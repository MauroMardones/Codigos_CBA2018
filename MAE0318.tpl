...
//5.Fases de estimación Lo y cv edad
  init_int     opt_Lo
  init_int     opt_cv
//6.Considera la matriz de asignación de error edad
  init_number  erredad
...
//Puntos biológicos de referencia
  ...
  init_int     npbr          // Número de puntos biológicos
  init_vector  ratio(1,npbr)
// 9. PROYECCIÓN, CRITERIOS DE EXPLOTACIÓN Y ESCENARIOS DE RECLUTAMIENTO
  init_int     nproy        // Número de años de proyección
...
// matrices y vectores de abundancias derivadas
 ...
 if(erredad==1) // corrección por error de asignación de la edad
  {
    ...
    Nvp  = Nv(nanos);
      ...
      { 
        ...
        Nvp(1) = exp(log_Ro+0.5*square(sigmaR));
              ...
              if(oprec==2) // reclutamiento promedio ultimos 8 años (recientes)
              {
       ...
       //------------------------------------------------------------------
        ...
        Fref = mF*log_Fref(1);
          ...
          if(opt_Str==0)// Regla de decisión------------
          {
              if(RPRp/0.6<1)
               {
